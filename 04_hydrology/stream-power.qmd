...
options(scipen=100) # scientific notation
```
...
combined <- rbind(wagga_monthly, gundagai_monthly, narrandera_monthly) |> 
  mutate(month=month_abb, month_abb=NULL, # months for plotting
         streampower = 10^log_streampower) |> # reference
  select("month", "station", "log_streampower", "streampower")
...
combined_sp <- rbind(wagga_monthly_sp, gundagai_monthly_sp, narrandera_monthly_sp) |> 
  mutate(month=month_abb, 
         month_abb=NULL,
         specific_sp=10^log_specific_sp) |>
  select("month", "station", "log_specific_sp", "specific_sp")

combined_sp
```

```{r}
combined_sp |> group_by(station, month) |>
  summarise(
    min=min(specific_sp),
    max=max(specific_sp)
  )
